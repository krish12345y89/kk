...
import { completeRegisterMail } from "../mail/server.js";
...
export const getPendingUser = async (req, res) => {
  try {
    const data = await User.find({ status: "Pending" });

    let object = data.map(({ uuid, name, email, rollNo, batch }, i) => {
      let obj = {
        id: i + 1,
        name: name,
        email: email,
...
export const sendPendingUserMail= async ()=>{
  const pnuser=await User.find({status:"Pending"});
  completeRegisterMail(pnuser.name,pnuser.email,pnuser.status);
}
